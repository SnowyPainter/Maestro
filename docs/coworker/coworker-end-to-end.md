...
CoWorker는 Persona 계정 단위로 반복적인 아웃바운드(이메일, 게시물)와 인바운드 상호작용을 자동화하는 시스템이다. 다음과 같은 설계 철학을 따른다:

### 1.1 Scheduler 설계 철학
- **외부 API는 Schedule row만 생성/취소**: 클라이언트는 스케줄 생성과 취소만 담당하고, 실제 실행 로직은 시스템 내부에서 처리한다.
- **모든 실행 로직은 internal.* 플로우 DAG로 표현**: 비즈니스 로직은 재사용 가능한 internal flow들의 조합으로 구성된다.
- **Registry는 internal 플로우 조합만 정의**: HTTP 의존성 없이 순수한 도메인 로직만 포함하여 재사용성을 보장한다.
- **Action layer는 사용자 맥락/검증/배치 계산에 집중**: 사용자 권한, 입력 검증, 배치 스케줄링 등 외부 인터페이스 관련 로직을 담당한다.
- **CoWorker는 Schedule + DagExecutor만 이해하면 됨**: 워커는 단순히 스케줄을 가져와서 DAG를 실행하는 역할에만 집중한다.
