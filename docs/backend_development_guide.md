...
- **Scheduler**: Adapter를 이용한 스케줄링된 작업 처리
- **Sniffer**: 외부 트렌드 데이터 수집 (Google Trends, Naver 등)
- **Synchro**: 개인화된 임베딩 엔진 (사용자별 → 계정별 메타데이터)
- **Adapter**: 플랫폼별 인터페이스 추상화 (게시/삭제/메트릭/데이터 전달)
- **CoWorker**: 이메일 루프 및 알림 기능
...
### 6.2 Scheduler 워커
- **역할**: 스케줄된 콘텐츠 발행
- **워크플로우**: 스케줄 조회 → Adapter 호출 → 발행 완료 이벤트
- **에러 처리**: 실패 시 이벤트 발송

### 6.3 기타 워커
- **Sniffer**: 상시 작동, 외부 트렌드 데이터 수집
- **Synchro**: 개인화 임베딩 엔진
- **CoWorker**: 이메일 루프, 알림 기능
