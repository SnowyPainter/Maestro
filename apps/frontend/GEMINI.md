...
* [`react-i18next`](https://react.i18next.com/) 사용 (React + TS 친화적, Suspense/동적 로딩 지원).
* 번역 리소스는 **JSON 파일 기반**.
...
## 4. 초기화 코드 (`src/lib/i18n/index.ts`)
...
## 5. Provider 설정 (`src/app/providers/i18n.tsx`)

```tsx
import "@/lib/i18n"; // 초기화 실행
...
## 6. 번역 파일 예시 (`src/lib/i18n/resources/en.json`)
